...
export class FinanceService {
  ...
  public getDebts(workerId?: string) {
  ...
  public recordDebtPayment(
    ...
    return this.dbService.transaction(() => {
      const debt = this.dbService.queryOne<any>(`SELECT * FROM debts WHERE id = ?`, [debtId]);
  ...
  public updateDebtStatus(debtId: string, status: DebtStatus, approvedBy: string, reason?: string) {
    return this.dbService.transaction(() => {
      const debt = this.dbService.queryOne<any>(`SELECT * FROM debts WHERE id = ?`, [debtId]);
      if (!debt) throw new NotFoundException('Debt record not found.');
  ...
  public processWorkerSalary(
    ...
    return this.dbService.transaction(() => {
      const worker = this.dbService.queryOne<any>(`SELECT * FROM workers WHERE id = ?`, [workerId]);
      ...
      const outstandingDebts = this.dbService.query<any>(
