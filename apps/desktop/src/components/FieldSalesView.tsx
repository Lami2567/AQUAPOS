    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 select-none">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="btn-touch bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-900/40 font-bold text-xs flex items-center gap-2"
        >
                    ...
                    <td className="p-3 text-right">
                      {session.status === 'OPEN' ? (
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-500 text-white px-3.5 py-1.5 rounded-lg font-bold text-xs shadow-md shadow-emerald-950 transition-all"
                        >
                          Close & Reconcile
                        </button>
                      ) : (
                        <span className="text-slate-500 text-[11px] font-semibold">Reconciled ✓</span>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {isStartModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full p-6 space-y-5 shadow-2xl text-slate-100 my-8">
            ...
            <form onSubmit={handleStartSessionSubmit} className="space-y-4 text-xs">
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-slate-400 font-mono">
                            Available in store: <span className="text-emerald-400 font-bold">{storeAvail}</span> {prod.unitOfMeasure} • UGX {prod.sellingPriceUgx.toLocaleString()}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="w-24 bg-slate-900 border border-slate-700 rounded-lg px-2.5 py-1.5 text-right font-mono font-bold text-cyan-300 focus:outline-none focus:border-cyan-500"
                          />
              ...
              <div className="flex gap-2 border-t border-slate-800 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2.5 rounded-xl shadow-lg shadow-cyan-900/40"
                >
      ...
      {activeReconcileSession && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-4xl w-full p-6 space-y-6 shadow-2xl text-slate-100 my-8">
            
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                <h2 className="text-lg font-extrabold text-cyan-400 flex items-center gap-2">
                  <FileSpreadsheet className="w-5 h-5" /> Field Session Reconciliation: {activeReconcileSession.sessionNumber}
                </h2>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                {activeReconcileSession.items.map((item) => {
                  ...
                  return (
                    <div key={item.productId} className="bg-slate-950 border border-slate-800 p-3.5 rounded-2xl space-y-2.5">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 font-mono text-emerald-300 font-bold"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 font-mono text-blue-300 font-bold"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 font-mono text-amber-300 font-bold"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 font-mono text-rose-300 font-bold"
                          />
                        </div>
                      </div>
            ...
            <div className="space-y-3 border-t border-slate-800 pt-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2.5 py-2 font-mono text-slate-100 font-bold"
                  />
                </div>
                <div>
              ...
              >
                <div>
                  <div>Accounted Total: UGX {moneyRes.totalAccountedUgx.toLocaleString()}</div>
            ...
            <div className="flex gap-3 border-t border-slate-800 pt-4">
              <button
                ...
                className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/30"
              >
                <CheckCircle className="w-4 h-4" /> Confirm & Authorize Reconciliation
              </button>
              <button
                onClick={() => setActiveReconcileSession(null)}
                className="bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold px-5 py-3 rounded-xl text-xs"
              >
                Cancel
              </button>
            </div>
